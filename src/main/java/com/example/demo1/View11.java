...
public class View11 extends goToButtons{
    ...
    private void onClickCheck() {
        ...
        if (((RadioButton)gr.getSelectedToggle()).getText().equals("Полный")) {
            boolean is_full = true;
            is_full = (is_full && (to_check.charAt(0)!='0'));
            is_full = (is_full && (to_check.charAt(to_check.length()-1)!='1'));
            is_full = (is_full && !check_S(to_check, to_check.length()/2));
            is_full = (is_full && !check_M(to_check, VectorSize));
            is_full = (is_full && !check_L(to_check, to_check.length()));
            if (is_full) {
        ...
        else if (((RadioButton)gr.getSelectedToggle()).getText().equals("Неполный")){
            ...
            String errorMessage = "Выберите свойство вектора";
            outCheck.setText(errorMessage);
            outCheck.setTextFill(Color.RED);
        }
    }
